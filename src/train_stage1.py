...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Stage 1 (CoT generator) with TAPEX on TABMWP"
    )
    parser.add_argument(
        "--config_path",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="checkpoints/stage1_tapex_large",
        help="Where to save TAPEX checkpoints.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def main() -> None:
    ...
    num_train_epochs = float(cfg.get("num_train_epochs", 25))
